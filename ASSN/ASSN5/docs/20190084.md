...
자식 프로세스가 현재 정지되어 있는 상태라면, 우선 job을 조회한다. 해당 Job이 존재하지 않는 경우에는 에러를 띄운다. 에러가 나지 않았다면, 해당 job의 상태를 ST로 설정하고, WSTOPSIG를 통해 자식 프로세스를 정지시킨 원인이 된 시그널을 출력한다.
...
```
 ...
 자식 프로세스가 정상적으로 종료된 상태일 때, 종료시킨 시그널이 0이 아니라면 에러를 띄운다. 에러가 발생하지 않았다면, job을 Job 목록에서 삭제한다.
...
```
 ...
 자식 프로세스가 어떤 시그널을 받아서 종료되었을 때이다. 이때, deletejob을 이용하여 우선 job을 삭제하고, 이후 WTERMSIG를 통해 종료의 원인이 된 시그널을 출력한다.
...
[2] (1929) ./myspin 3 &
tsh> jobs
[1] (1927) Running ./myspin 2 &
[2] (1929) Running ./myspin 3 &
...
[2] (1940) ./myspin 3 &
tsh> jobs
[1] (1938) Running ./myspin 2 &
[2] (1940) Running ./myspin 3 &
```
...
#
tsh> ./myspin 4
Job [1] (2043) terminated by signal 2
./sdriver.pl -t trace06.txt -s ./tsh -a "-p"
#
# trace06.txt - Forward SIGINT to foreground job.
#
tsh> ./myspin 4
Job [0] (2059) terminated by signal 2
```
...
[1] (2427) ./myspin 4 &
tsh> ./myspin 5
Job [2] (2429) terminated by signal 2
tsh> jobs
[1] (2427) Running ./myspin 4 &
...
[1] (2439) ./myspin 4 &
tsh> ./myspin 5
Job [0] (2441) terminated by signal 2
tsh> jobs
[1] (2439) Running ./myspin 4 &
```
